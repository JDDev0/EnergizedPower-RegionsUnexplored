...
public class SiltFarmlandCraftingRecipe extends CustomRecipe {
    public SiltFarmlandCraftingRecipe(CraftingBookCategory category) {
        super(category);
    }
    ...
    public boolean matches(CraftingInput container, Level level) {
        ...
        for(int i = 0;i < container.size();i++) {
            ...
            if(!itemStack.isEmpty()) {
                if(itemStack.is(RuBlocks.SILT_DIRT.get().asItem())) {
                    dirtCount++;
    ...
    public ItemStack assemble(CraftingInput container, HolderLookup.Provider registries) {
        return new ItemStack(RuBlocks.SILT_FARMLAND.get());
    }
    ...
    public RecipeSerializer<?> getSerializer() {
        return EPRURecipes.SILT_FARMLAND_CRAFTING_RECIPE_SERIALIZER.get();
    }
